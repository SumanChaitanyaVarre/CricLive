...
const Bowl_scorecard = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                let res = await fetch("http://localhost:8080/ball_score/"+match_id, {
                    method: 'POST',
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        <thead class="thead-dark">
                            <tr>
                                ...
                                <th style={{ border: "1px solid black" }}>runs given</th>
                                <th style={{ border: "1px solid black" }}>wickets</th>
                                <th style={{ border: "1px solid black" }}>over.ball</th>
                        ...
                        <tbody>
                            {score && score.map(item => (
                                <tr key={ item.player_id}>
                                    ...
                                    <td style={{ border: "1px solid black" }}>{item.runs_given}</td>
                                    <td style={{ border: "1px solid black" }}>{item.wickets}</td>
                                    <td style={{ border: "1px solid black" }}>{item.over_id}.{item.ball_id} </td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
